...
public class MeshLineRenderer : MonoBehaviour
{
    ...
    public Material lmat; // define line material

    private Mesh ml; // mesh

    private Vector3 s; // keeps track of previous start point

    private float lineSize = .04f; // line size

    private bool firstQuad = true; // check if this is the first drawn point

    void Start()
    {
        ml = GetComponent<MeshFilter>().mesh; // keeps track of mesh
        GetComponent<MeshRenderer>().material = lmat; // visualizes mesh
    }
    ...
    public void AddPoint(Vector3 point) // checks to make sure start is not at 0
    {
        if (s != Vector3.zero)
        {
    ...
    Vector3[] MakeQuad(Vector3 s, Vector3 e, float w, bool all) // drawing verticies based on two points
    {
        w = w / 2;
